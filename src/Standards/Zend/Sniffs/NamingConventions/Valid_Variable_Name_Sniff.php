 ...
 */
namespace Php_code_Sniffer\Standards\Zend\Sniffs\Naming_Conventions;
...
class Valid_Variable_Name_Sniff extends Abstract_Variable_Sniff
{
            ...
            if ($tokens[$var]['code'] === T_STRING) {
                ...
                if ($tokens[$bracket]['code'] !== T_OPEN_PARENTHESIS) {
                    ...
                    if (Common::is_camel_caps($obj_var_name, false, true, false) === false) {
                        ...
                        $phpcs_file->add_error($error, $var, 'NotCamelCaps', $data);
                    } elseif (preg_match('|\d|', $obj_var_name) === 1) {
                        $warning = 'Variable "%s" contains numbers but this is discouraged';
                        $data = [$original_var_name];
                        $phpcs_file->add_warning($warning, $stack_ptr, 'ContainsNumbers', $data);
                    }
                }
                //end if
            }
    ...
    protected function process_member_var(File $phpcs_file, $stack_ptr)
    {
        ...
        if (Common::is_camel_caps($var_name, false, true, false) === false) {
            ...
            $data = [$var_name];
            $phpcs_file->add_error($error, $stack_ptr, 'MemberVarNotCamelCaps', $data);
        } elseif (preg_match('|\d|', $var_name) === 1) {
            $warning = 'Member variable "%s" contains numbers but this is discouraged';
            $data = [$var_name];
            $phpcs_file->add_warning($warning, $stack_ptr, 'MemberVarContainsNumbers', $data);
        }
    }
        ...
        if (preg_match_all('|[^\\\\]\$([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)|', $tokens[$stack_ptr]['content'], $matches) !== 0) {
            foreach ($matches[1] as $var_name) {
                ...
                if (Common::is_camel_caps($var_name, false, true, false) === false) {
                    $error = 'Variable "%s" is not in valid camel caps format';
                    $data = [$var_name];
                    $phpcs_file->add_error($error, $stack_ptr, 'StringVarNotCamelCaps', $data);
                } elseif (preg_match('|\d|', $var_name) === 1) {
                    $warning = 'Variable "%s" contains numbers but this is discouraged';
                    $data = [$var_name];
                    $phpcs_file->add_warning($warning, $stack_ptr, 'StringVarContainsNumbers', $data);
                }
            }
            //end foreach
        }
