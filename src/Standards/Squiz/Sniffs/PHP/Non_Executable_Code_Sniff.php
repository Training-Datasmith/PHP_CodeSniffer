...
class Non_Executable_Code_Sniff implements Sniff
{
    /**
     ...
     * @var array
     */
    private $expression_tokens = [T_EXIT => T_EXIT, T_THROW => T_THROW];
    /**
    ...
    public function process(File $phpcs_file, $stack_ptr)
    {
        ...
        if (isset($this->expression_tokens[$tokens[$stack_ptr]['code']]) === true) {
            ...
            if (isset(Tokens::$boolean_operators[$tokens[$prev]['code']]) === true && ($tokens[$stack_ptr]['code'] === T_THROW && $tokens[$prev]['code'] === T_LOGICAL_XOR) === false) {
                return;
            }
        ...
        if ($tokens[$stack_ptr]['code'] === T_RETURN) {
            ...
            if ($tokens[$next]['code'] === T_SEMICOLON) {
                ...
                if ($tokens[$next]['code'] === T_CLOSE_CURLY_BRACKET) {
                    ...
                    if ($tokens[$owner]['code'] === T_FUNCTION) {
                        ...
                        $phpcs_file->add_warning($warning, $stack_ptr, 'ReturnNotRequired');
                        return;
                    }
                }
            }
        ...
        if (isset($tokens[$stack_ptr]['scope_opener']) === true) {
            $owner = $tokens[$stack_ptr]['scope_condition'];
            if ($tokens[$owner]['code'] === T_CASE || $tokens[$owner]['code'] === T_DEFAULT) {
                ...
                $next = $phpcs_file->find_next([T_CASE, T_DEFAULT, T_CLOSE_CURLY_BRACKET, T_ENDSWITCH], $end + 1);
                if ($next !== false) {
                    $last_line = $tokens[$end]['line'];
                    for ($i = $stack_ptr + 1; $i < $next; $i++) {
                        ...
                        $line = $tokens[$i]['line'];
                        if ($line > $last_line) {
                            $type = substr($tokens[$stack_ptr]['type'], 2);
                            $warning = 'Code after the %s statement on line %s cannot be executed';
                            $data = [$type, $tokens[$stack_ptr]['line']];
                            $phpcs_file->add_warning($warning, $i, 'Unreachable', $data);
                            $last_line = $line;
                        }
                    }
                }
        ...
        $our_conditions = array_keys($tokens[$stack_ptr]['conditions']);
        if (empty($our_conditions) === false) {
            $condition = array_pop($our_conditions);
            ...
            if ($tokens[$condition]['code'] === T_SWITCH && $tokens[$stack_ptr]['code'] === T_BREAK) {
                return;
            }
            ...
            if ($tokens[$stack_ptr]['code'] === T_BREAK) {
                return;
            }
            // Throw an error for all lines until the end of the file.
            $end = $phpcs_file->num_tokens - 1;
        }
        //end if
        // Find the semicolon or closing PHP tag that ends this statement,
        // skipping nested statements like FOR loops and closures.
        for ($start = $stack_ptr + 1; $start < $phpcs_file->num_tokens; $start++) {
            if ($start === $end) {
                break;
            }
            if (isset($tokens[$start]['parenthesis_closer']) === true && $tokens[$start]['code'] === T_OPEN_PARENTHESIS) {
                $start = $tokens[$start]['parenthesis_closer'];
                continue;
            }
            if (isset($tokens[$start]['bracket_closer']) === true && $tokens[$start]['code'] === T_OPEN_CURLY_BRACKET) {
        ...
        $last_line = $tokens[$start]['line'];
        for ($i = $start + 1; $i < $end; $i++) {
            if (isset(Tokens::$empty_tokens[$tokens[$i]['code']]) === true) {
                continue;
            }
            if (isset(Tokens::$bracket_tokens[$tokens[$i]['code']]) === true) {
                continue;
            }
            ...
            if (isset(Tokens::$oo_scope_tokens[$tokens[$i]['code']]) === true || $tokens[$i]['code'] === T_FUNCTION || $tokens[$i]['code'] === T_CLOSURE) {
